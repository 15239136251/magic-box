<template>
    <view class="store-allocation-detail container overflow-y-auto flex flex-col">
        <!-- 单头 -->
        <view class="w-full bg-white text-sm relative">
            <view class="p-2">
				<view class="flex mb-3">
					<text class="w-20">单据编号：</text>
					<text>{{ detail.docno || '系统自动生成' }}</text>
				</view>
				<view class="flex mb-3">
					<text class="w-20">收货门店：</text>
				...
				<view class="flex mb-3">
					<text class="w-20">备注：</text>
					<textarea :disabled="disabled" :value="detail.remark" @input="remarkInput" class="w-48 h-10 p-1 text-sm bg-gray-200"></textarea>
				</view>
			</view>
            <view class="p-2">
        ...
        <view class="detail-button w-full flex items-center justify-around" style="z-index: 99;">
			...
			<view class="flex detail-button-wrap py-3 mr-2" >
				...
				<button plain="true" size="mini" class="rounded-full mr-2" v-if="disabled" @click="flushRed">冲红</button>
...
// 参数
const { detail, expcomlist, disabled, totQty, submitDialog } = toRefs(allocationStore)
// 方法
const { getDetail, getStoreList, storeInput, remarkInput, setSearchValue, delItem, inputnumberChange, expcomInput, fastNoInput, fastNoScan, out, save, submit, submitDialogOk, submitDialogClose, flushRed, getExpcom } = allocationStore

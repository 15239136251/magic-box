<template>
    <view class="store-in-list container overflow-y-auto flex flex-col">
        <!-- 顶部筛选 -->
                ...
                <template v-if="list.length">
                    <view class="bg-white rounded overflow-hidden relative p-3 mb-2"  v-for="item in list" :key="item.id">
                        ...
                        <view class="flex mb-2">
                            ...
                            <view class="flex-auto">
                                <text>快递信息：</text>
                                <text>{{ item.express_info || '' }}</text>
                            </view>
                        </view>
                        <view class="flex mb-2">
                            <view class="flex-auto">
                                <text>物流信息：</text>
                                <text>{{ item.kd100_lastest_ifno || '' }}</text>
                            </view>
                        </view>
                        <view class="flex mb-2">
                            <view class="flex-auto">
                                <text>入库日期：</text>
                                <text>{{ item.datein || '' }}</text>
                            </view>
                        </view>
                        <view class="flex mb-2">
                        ...
                        <view class="flex items-center justify-between pt-2 border-t border-solid border-x-0 border-b-0 border-gray-200">
                            <view class="">
                                <text>共计{{ item.tot_qtyout || 0 }}件，入库数量{{ item.tot_qtyin || 0}}件</text>
                            </view>
...
onShow(() => {
    console.log('storeInList ---> onShow')
    listCover()
})

const inStore = useInStore()
// 参数
const { range, searchValue, status, filterOptions } = toRefs(inStore)
// 方法
const { searchInput, goto } = toRefs(inStore)
...
interface InStoreApi {
    id: number
    docno: string
    billdate: number
    origname: string
    express_info: number
    kd100_lastest_ifno: string | null
    datein: string | null
    description: string,
    modifieddate: string
    tot_qtyin: number
...
const inlist = useList<InStoreApi, Object>(getList, filterOptions)
// 参数

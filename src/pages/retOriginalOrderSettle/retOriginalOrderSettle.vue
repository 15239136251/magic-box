				...
				>
					...
					<view class="flex flex-col justify-between ml-4 flex-1 w-32">
						<view class="">
							...
							<text class="text-xs bg-gray-100 rounded px-1 ml-1" v-if="beAmts[pay.name]">{{ '可退: ' + beAmts[pay.name] }}</text>
						</view>
						<view class="w-16">
...
import { useRetOriginalOrderSettleStore } from '@/stores/ret-original-order-settle'

const retOriginalOrderSettleStore = useRetOriginalOrderSettleStore()

/* 参数 */
const { totAmt, amtinv, beAmtinv, payItems, payType, beAmts, payAmts, payAllAmt, routerValue, isOpenVipStoredValue } = toRefs(retOriginalOrderSettleStore)
/* 方法 */
const { init, payItemClick, isPayShow, payAmtsInput, out, rePay, save, getPays, vipStoredValueSave, closeVipStoredValue } = retOriginalOrderSettleStore

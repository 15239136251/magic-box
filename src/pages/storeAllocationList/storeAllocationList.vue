<template>
	<view class="store-allocation-list container overflow-y-auto flex flex-col">
		<!-- 顶部筛选 -->
		...
		<view class="h-full overflow-hidden text-sm flex flex-col items-center px-2 py-4">
			...
			<view class="w-full h-8 fixed left-0 bottom-0 bg-white flex flex-center py-2">
				<view class="flex flex-center px-2 w-full">
					<view class="w-full rounded bg-red-500 flex flex-center text-white" style="height: 60rpx; line-height: 60rpx;" @click="toAdd">新增快速调拨</view>
				</view>
			</view>
...
import { useAllocationStore } from '@/stores/store-allocation'

onShow(() => {
    console.log('storeAllocationList ---> onShow')
    listCover()
})

const allocationStore = useAllocationStore()
// 参数
const { range, searchValue, status, filterOptions } = toRefs(allocationStore)
// 方法
const { searchInput, goto, toAdd } = toRefs(allocationStore)
...
const getList = async (data: any) => {
    let value = await REQ({
        url: 'pos/transfer/list',
		method: 'POST',
        data
    })
    return value
}
interface allocationStoreApi {
    id: number
    docno: string
    billdate: number
	destname: string
...
const allocationlist = useList<allocationStoreApi, Object>(getList, filterOptions)
// 参数
const { loading, list, total, curPage, isEnd } = allocationlist
// 方法
const { filter } = allocationlist
...
const search = () => {
    ...
    console.log("🚀 ~ file: storeAllocationList.vue:196 ~ search ~ filterOptions", filterOptions.value)
    debounce(listCover, {})
}
...
const lower = () => {
    ...
    console.log("🚀 ~ file: storeAllocationList.vue:208 ~ lower ~ 到达底部")
    debounce(() => {

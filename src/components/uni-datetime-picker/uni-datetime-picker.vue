<template>
	<view class="uni-date">
		...
		'uni-date-x--border': border}">
					<view v-if="!isRange" class="uni-date-x uni-date-single" :style="{'background-color': noBg ? '' : '#F3F4F6;'}">
					...
					<view v-else class="uni-date-x uni-date-range" :style="{'background-color': noBg ? '' : '#F3F4F6;'}">
						<uni-icons type="calendar" color="#e1e1e1" size="22"></uni-icons>
					...
					<view v-if="showClearIcon" class="uni-date__icon-clear" @click.stop="clear">
						<uni-icons type="clear" color="#ffffff" size="18"></uni-icons>
					</view>
				</view>
			</slot>
		</view>
	...
	export default {
		...
		methods: {
			...
			setEmit(value) {
				...
				if (this.returnType === 'number' && this.type === 'daterange') {
					value[0] = value[0].split('-').join('')
					value[1] = value[1].split('-').join('')
					console.log("value", value)
				} else if (this.returnType === 'number' && this.type === 'date') {
